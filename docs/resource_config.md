...
| 遅延post の pending 表 | 720 B | `DELAY_SLOTS × (event_t + due + flags)`（≈180 B/枠, v0.4.7） |
...
| `CFG_DELAY_SLOTS` | `×180 B/枠`（v0.4.7） | **同時に待てる遅延post の本数**（`-> ハンドラ AFTER <ms>`）。溢れると新着を捨てて `ERR_DELAY_FULL`。実機で RAM が厳しければ 2 へ |
...
| `CFG_SARG_LEN` | `SARG` ＋ `save_sarg × CALL_NEST` ＋ `event.sstr × (QUEUE_LEN + DELAY_SLOTS)` | **受信文字列の最大長**。式 ≈ `SARG_LEN × SARG_COUNT × (1 + CALL_NEST + QUEUE_LEN + DELAY_SLOTS)` |
| `CFG_SARG_COUNT` | 同上（`× SARG_LEN`） | 1メッセージに載せられる**文字列位置の数**（`(int,int,str)` の str 位置） |
| `CFG_ARG_COUNT` | `ARG` ＋ `save_arg×CALL_NEST` ＋ `event.pos×(QUEUE_LEN + DELAY_SLOTS)`（`×8`） | 1メッセージ/呼び出しの**引数位置数** |
...
1. **大物3つ**（`CFG_MAX_PORTS` / `CFG_EVENT_QUEUE_LEN` / `CFG_CALL_NEST`）を実需まで下げる。ポート表が最大の項、キューとコールスタックが続きます。`CFG_DELAY_SLOTS`（遅延post の同時本数）も同種で、2 に絞れば −360 B。
