 ...
 * 値の型は int / str の2択（v0.4.2で内部 CHAR タグを撤去し純化・§9）。
 * 型タグは「出力先（my_print_f等）が数値/文字列を出し分ける」ために保持する（§1, §10）。
 *   - VT_INT  : 整数。.i に値。文字リテラル 'A' も int 65 として乗る（§2）。
 *   - VT_STR  : 文字列定数参照。.i は文字列プール内オフセット（script_str()で解決）。
 * 「数を文字グリフとして描く」のは値ではなく出力側の仕事（CHR / FORMATTER %c・§3）。
 *
 * ホスト非依存（純C）コア層。
 */
#ifndef VALUE_H
...
typedef enum {
    SV_INT  = 0,   /* 整数（文字リテラル 'A'=65 もこれ, v0.4.2）。CHAR タグは撤去（§9） */
...
typedef struct {
    value_type_t tag;
    int32_t      i;   /* INT:値 / STR:プールoff / SREF:(kind<<8)|idx */
} value_t;
