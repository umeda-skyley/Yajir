/* main.c - PC拡張ホストのエントリ＋コンパイルチェック（仕様 §1 メインループ）
 *
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3 || (strcmp(argv[1], "run") != 0 && strcmp(argv[1], "check") != 0)) {
        fprintf(stderr, "usage: %s run <script.txt> [seconds]\n", argv[0]);
        fprintf(stderr, "       %s check <script.txt>\n", argv[0]);
        return 2;
    }
    check_only = (strcmp(argv[1], "check") == 0);
    if (check_only && argc != 3) {
        fprintf(stderr, "usage: %s check <script.txt>\n", argv[0]);
        return 2;
    }
    path = argv[2];
    if (!check_only && argc >= 4) max_ms = atoi(argv[3]) * 1000;
